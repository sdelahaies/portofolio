import enpgf from '/public/image/enpgf.jpg';
import spacequest from '/public/image/spacequest.jpg';
import trip from '/public/image/trip.png';
import chatgpt from '/public/image/chatgpt.png';
import chatgpt2 from '/public/image/chatgpt2.png';
import portfolio from '/public/image/portfolio.png';
import GUIassistant from '/public/image/voice.jpg';
import flowizer from '/public/image/flowizer-terminal.png';
import flowizer_ui from '/public/image/flowizer-ui.png';
import catdog from '/public/image/catdog.jpg';
import llmEval from '/public/image/llm-eval.png';
import htr from '/public/image/htr-condat.jpg';

export const blogData = [
    {
        id: 1,
        url: "https://github.com/sdelahaies/htr-archives", //"/blog/flowizer/"
        title: "HTR tools & models",
        description: "Entraînement de modèles et développement d'outils pour l'étude d'archives manuscriptes via HTR (Handwritten Text Recognition). Scipts d'entraînement pour divers modèles, création de jeux de données synthétiques, UI pour l'exploitation des résultats.",
        image: htr
    },
    {
        id: 2,
        url: "https://github.com/sdelahaies/llm-eval", //"/blog/flowizer/"
        title: "LLM Eval",
        description: "LLM evaluation with hands-on notebooks for LightEval, DeepEval, and HumanEval— a guide to benchmarking AI performance effectively.",
        image: llmEval
    },
    {
        id: 3,
        url: "https://sdelahaies.github.io/laser-llm.html", //"/blog/flowizer/"
        title: "TSVD & LLM",
        description: "Optimizing language models through LASER, quantization, and fine-tuning for efficient, high-performance, and scalable AI solutions.",
        image: catdog
    },
    {
        id: 4,
        url: "https://sdelahaies.github.io/flowizer-ui.html", //"/blog/flowizer/"
        title: "Flowizer-UI: A Low-Code Interface for Workflow Automation",
        description: "Flowizer-ui is a Next.js ReactFlow front-end application that allows you to visualize the data generated by the Flowizer workflow engine API.",
        image: flowizer_ui
    },
    {
        id: 5,
        url: "https://sdelahaies.github.io/flowizer.html", //"/blog/flowizer/"
        title: "Flowizer: Agentic Workflow Automation",
        description: "A lightweight Python package designed for creating, managing, and running workflows represented as directed acyclic graphs (DAGs).",
        image: flowizer
    },
    {
        id: 6,
        url: "https://sdelahaies.github.io/iassistant.html",//"/blog/iassistant/"
        title: "IAssistant: A Versatile AI Voice Assistant for Developers and Power Users",
        description: "Artificial Intelligence (AI) continues to transform how we interact with technology. Voice assistants, in particular, offer a natural way to engage with complex systems.",
        image: GUIassistant
    },
    {
        id: 7,
        url: "https://sdelahaies.github.io/enpgf-lab.html",
        title: "Online EnPGF training for temporal point processes",
        description: "Online training framework for a pytorch implementation of the Ensemble Poisson Gamma Filter using Apache Kafka.",
        image: enpgf
    },
    {
        id: 8,
        url: "https://sdelahaies.github.io/deepwars.html",
        title: 'Object Localization in Space... kinda',
        description: "Exploration de techniques de deep learning pour la détection d'objets. Implémentation et entrainement de modèle tensorflow, transfert d'apprentissage avec YOLOv5 et DETR.",
        image: spacequest,
    },
    {
        id: 9,
        url: "https://sdelahaies.github.io/travelplanner.html",
        title: "Recommandation d'itinéraire de vacances",
        description: "Création d'une application de recommandation d'itinéraire de vacances: conception, implémentation, déploiement.",
        image: trip,
    },
    {
        id: 10,
        url: "https://sdelahaies.github.io/chatgpt-prompts2.html",
        title: "Is ChatGPT ruining my Data Engineer career before it even started? TAKE 2.",
        description: "How to leverage chatgpt to start with Node.js.",
        image: chatgpt2
    },
    {
        id: 11,
        url: "https://sdelahaies.github.io/chatgpt-prompts.html",
        title: 'Is Chatgpt ruining my Data Engineer career before it even started?',
        description: "How to leverage chatgpt to write python code.",
        image: chatgpt,
    },
    {
        id: 12,
        url: "https://sdelahaies.github.io",
        title: 'Github portfolio',
        description: "version précédente de mon portfolio.",
        image: portfolio,
    }
];

